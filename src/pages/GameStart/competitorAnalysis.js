...
class CompetitorAnalysis extends Component {
  ...
  render() {
    const { profitPerCompany, accumprofitPerCompany, roundArray } = this.state;
    if (roundArray) {
      this.firmPanes = Object.keys(roundArray).map(key => {
        return {
          menuItem: key,
          render: () => {
            return (
